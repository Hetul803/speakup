...
@router.get("/{child_id}/notes", response_model=list[schemas.CaregiverNoteResponse])
def list_notes(child_id: int, category: str | None = None, db: Session = Depends(get_db)):
    child = crud.get_child(db, child_id)
    if not child:
        raise HTTPException(status_code=404, detail="Profile not found")
    return crud.get_caregiver_notes(db, child_id, category=category)
